...
test('chunks data into lines and passes on each line as one token, not tracking lines since verbose is false', () => {
  ...
  const failEarly = anything()                       // is not used by line lexer, should not make any difference
  const argv      = anything()                       // is not used by line lexer, should not make any difference
  ...
  assert(
    property(failEarly, argv, tokens, rest, offset, (failEarly, argv, tokens, rest, offset) => {
      const data     = tokens.join('\n') + (tokens.length > 0 ? '\n' : '') + rest
      const lastLine = offset
      const lines    = []

      expect(
        lexer(verbose, failEarly, argv)(data, offset)
      ).toStrictEqual(
